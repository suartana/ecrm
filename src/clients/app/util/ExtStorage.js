...
Ext.define('Docucrm.util.ExtStorage', {
	singleton: true,
	alternateClassName: ['ExtStorage'],
	//uses: ['System.Dialog', 'Base.view.mixins.SmartComponent'],
	/**
	 * The id of the Docucrm component.
	 */
	id: 'extstorage',// base storage class to use, must implement the localStorage API.
	_storage: null,
	...
	getTimeLeft: function(key) {
		...
		var expireKey = this._storage.getItem(this._expiration_key_prefix + key),
			expireTime = parseInt(expireKey);
	...
	isExpired: function(key) {
		...
		// return if expired
		return timeLeft !== null && timeLeft <= 0 ? false : true;
	},
	...
	keys: function(includeExpired) {
		...
		this._iterKeys(function(storageKey) {
			...
			if (storageKey.indexOf(me._expiration_key_prefix) !== 0) {

				// add to return list, but only if including expired keys or if not expired yet
				if (includeExpired || !me.isExpired(storageKey)) {
					ret.push(storageKey);
				}
			}
		});
	...
	clearExpired: function() {
		...
		this._iterKeys(function(storageKey) {
			...
			if (storageKey.indexOf(me._expiration_key_prefix) === 0) {
				...
				// get item key
				var itemKey = storageKey.substr(me._expiration_key_prefix.length);
				console.log("itemKey",itemKey);
				// if expired remove it + the item
				if (me.isExpired(itemKey)) {
					console.log("remove expired key",itemKey);
					me.removeItem(itemKey);
					ret.push(itemKey);
				}else{
					me.removeItem(itemKey);
					ret.push(itemKey);
